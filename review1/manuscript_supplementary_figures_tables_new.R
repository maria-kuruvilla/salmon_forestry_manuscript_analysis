...
#check how many of the watersheds in forestry_data_timeseries is in 
# the ch20rsc and the pk10r datasets

full_lookup <- lookup_og %>% 
  # filter(Species == "CM") %>% 
  select(LINEAR_FEATURE_ID, GFE_ID) %>% 
  distinct() %>% 
...
ch20rsc %>% 
  select(GFE_ID) %>% 
  filter(!GFE_ID %in% full_lookup$GFE_ID)

pk10r %>% 
  select(GFE_ID) %>% 
  filter(!GFE_ID %in% full_lookup$GFE_ID)

# all watersheds in the two datasets are in the full lookup dataset
full_lookup %>% 
  select(GFE_ID) %>% 
  distinct() %>% 
  filter(GFE_ID %in% ch20rsc$GFE_ID | GFE_ID %in% pk10r$GFE_ID) %>% 
  summarize(n())
#588
...
write.csv(summary_cpd_chum, here("output_figures_tables",
                                 "supplementary_table4_sep2026_chum_cpd_diagnostics_by_parameter_group.csv"),
          row.names = FALSE)
...
write.csv(summary_eca_chum, here("output_figures_tables",
                                 "supplementary_table4_sep2026_chum_eca_diagnostics_by_parameter_group.csv"),
          row.names = FALSE)
...
recruitment_decline_df_new <- function(posterior, effect, species, covariate_value){
  ...
  if(effect == "eca"){
    ...
    no_forestry = (min((sqrt(df$ECA_age_proxy_forested_only)-mean(sqrt(df$ECA_age_proxy_forested_only)))/sd(sqrt(df$ECA_age_proxy_forested_only))))
  ...
  recruitment_df = data.frame(species = species,
                               ...
                               recruitment_median = round(median(recruitment_decline),2),
                               recruitment_025 = round(quantile(recruitment_decline, probs = 0.025),2),
                               recruitment_975 = round(quantile(recruitment_decline, probs = 0.975),2)
  )
...
current_average_cpd_2022 <- ch20rsc %>% 
  left_join(full_lookup, by = c("GFE_ID")) %>% 
  group_by(River_n) %>%
...
current_average_cpd_pink_2022 <- pk10r %>% 
  left_join(lookup_og %>% 
...
max_average_eca_pink_2022 <- pk10r %>%
  left_join(lookup_og %>% 
              filter(Species == "PKO" | Species == "PKE") %>% 
              select(LINEAR_FEATURE_ID, GFE_ID) %>% distinct(), by = c("GFE_ID")) %>%
...
chum_recruitment_decline_cpd <- recruitment_decline_df_new(ric_chm_cpd_ocean_covariates_logR_long_chain,
                                                         effect = "cpd",
                                                         species = "chum",
                                                         covariate_value = current_average_cpd$current_average_cpd)

chum_recruitment_decline_cpd_2022 <- recruitment_decline_df_new(ric_chm_cpd_ocean_covariates_logR_long_chain,
                                                           effect = "cpd",
                                                           species = "chum",
                                                           covariate_value = current_average_cpd_2022$current_average_cpd_2022)


chum_recruitment_decline_eca <- recruitment_decline_df_new(ric_chm_eca_ocean_covariates_logR_long_chain,
                                                         effect = "eca",
                                                         species = "chum",
...
pink_recruitment_decline_cpd <- recruitment_decline_df_new(ric_pk_cpd_ersst_long_chain,
                                                         effect = "cpd",
                                                         species = "pink",
                                                         covariate_value = current_average_cpd_pink$current_average_cpd)

pink_recruitment_decline_cpd_2022 <- recruitment_decline_df_new(ric_pk_cpd_ersst_long_chain,
                                                           effect = "cpd",
                                                           species = "pink",
                                                           covariate_value = current_average_cpd_pink_2022$current_average_cpd_2022)

pink_recruitment_decline_eca <- recruitment_decline_df_new(ric_pk_eca_ersst_long_chain,
                                                         effect = "eca",
                                                         species = "pink",
                                                         covariate_value = max_average_eca_pink$max_average_eca)

pink_recruitment_decline_eca_2022 <- recruitment_decline_df_new(ric_pk_eca_ersst_long_chain,
                                                           effect = "eca",
                                                           species = "pink",
                                                           covariate_value = max_average_eca_pink_2022$max_average_eca_2022)
...
recruitment_decline_river_df_new <- function(posterior, effect, species){
  ...
  for (i in 1:length(unique(df$River_n))){
    ...
    cpd_river_2022 <- max(river_data$haarea_prct_cs_2022)
    ...
    high_forestry <- cpd_sqrt_std_river
    
    no_forestry <- min(df$sqrt.CPD.std)
    
    forestry_diff_2022 <- sqrt(cpd_river_2022)/sd(df$sqrt.CPD)
...
#save 
ggsave(here("output_figures_tables","supplementary_fig28_sep2026_chum_ricker_cpd_recruitment_decline_by_river_forest_plot_2022.png"), width = 8, height = 10)
...
case_study_watersheds_data <- ch20rsc_2022 %>% 
...
plot_recruitment_change_river_together_new(
  posterior1 = ric_chm_cpd_ocean_covariates_logR_long_chain,
  posterior2 = ric_chm_eca_ocean_covariates_logR_long_chain,
  river_name = "VINER SOUND CREEK",
  effect1 = "cpd",
  effect2 = "eca",
  species = "chum",
  model1 = "CPD",
...
for(i in watersheds){
  ...
  ggsave(filename = here("output_figures_tables",
                         paste0("supplementary_fig_case_study_sep2026_",str_replace_all(str_to_lower(i), " ", "_"),".png")),
...
case_study_watersheds_data_pk <- pk10r_2022 %>% 
...
for(i in watersheds){
  ...
  cda_diff <- sqrt(max(river_data$disturbedarea_prct_cs))/sd(ch20rsc_2022$sqrt.CPD)
  
  eca_diff <- sqrt(max(river_data$ECA_age_proxy_forested_only))/sd(ch20rsc_2022$sqrt.ECA)
  
  cda_diff_2022 <- sqrt(max(river_data$haarea_prct_cs_2022))/sd(ch20rsc_2022$sqrt.CPD)
  
  eca_diff_2022 <- sqrt(max(river_data$ECA_age_proxy_forested_only_max))/sd(ch20rsc_2022$sqrt.ECA)
  ...
  recruitment_cpd_2022 <- (exp(as.matrix(posterior_cpd_df %>% filter(Effect == "for") %>% select(coefficient))%*%
                            (cda_diff_2022)))*100 - 100
  
  recruitment_eca_2022 <- (exp(as.matrix(posterior_eca_df %>% filter(Effect == "for") %>% select(coefficient))%*%
                            (eca_diff_2022)))*100 - 100
  ...
  recruitment_df <- data.frame(
    ...
    lower_95 = c(round(quantile(recruitment_cpd, 0.025),1), round(quantile(recruitment_eca, 0.025),1)),
    upper_95 = c(round(quantile(recruitment_cpd, 0.975),1), round(quantile(recruitment_eca, 0.975),1))
  )
  
  recruitment_df_2022 <- data.frame(
    Effect = c("Recruitment change at max CPD level (2022)", "Recruitment change at max ECA level (2022)"),
    model = c("CDA", "ECA"),
    median = c(round(median(recruitment_cpd_2022),1), round(median(recruitment_eca_2022),1)),
    lower_95 = c(round(quantile(recruitment_cpd_2022, 0.025),1), round(quantile(recruitment_eca_2022, 0.025),1)),
    upper_95 = c(round(quantile(recruitment_cpd_2022, 0.975),1), round(quantile(recruitment_eca_2022, 0.975),1))
  )
...
for(i in pink_watersheds){
  ...
  cda_diff <- sqrt(max(river_data$disturbedarea_prct_cs))/sd(pk10r_2022$sqrt.CPD)
  
  eca_diff <- sqrt(max(river_data$ECA_age_proxy_forested_only))/sd(pk10r_2022$sqrt.ECA)
  
  cda_diff_2022 <- sqrt(max(river_data$haarea_prct_cs_2022))/sd(pk10r_2022$sqrt.CPD)
  
  eca_diff_2022 <- sqrt(max(river_data$ECA_age_proxy_forested_only_max))/sd(pk10r_2022$sqrt.ECA)
...
all_river_df_chum_pink <- all_river_df %>% 
  ...
  mutate(Effect = case_when(Effect == "for" ~ "Forestry effect size",
                            ...
                            Effect == "Recruitment change at max ECA level (2022)" ~ "Productivity change (2022)"
                            )) %>%
  rename(Model = model) %>%
...
# look at which CUs have the lowest estimated effect of NPGO

ric_chm_cpd_ocean_covariates_logR_long_chain %>% 
  select(starts_with('b_npgo_cu')) %>%
  pivot_longer(cols = everything(),
               names_to = c('Effect','CU'),
               names_pattern = 'b_(.*)_cu(.*)',
               values_to = "coefficient") %>%
  ...
  arrange(median) %>%
  left_join(ch20rsc %>% select(CU, CU_NAME, CU_n) %>% distinct(), by = c("CU_n" = "CU_n")) %>%
  select(CU, CU_NAME, median, lower_95, upper_95) %>% 
  View()
   
# for  pink

ric_pk_cpd_ersst_long_chain %>% 
  select(starts_with('b_npgo_cu')) %>%
  pivot_longer(cols = everything(),
               names_to = c('Effect','CU'),
               names_pattern = 'b_(.*)_cu(.*)',
  ...
  summarise(median = median(coefficient),
            ...
            upper_95 = quantile(coefficient, 0.975)) %>%
  arrange(median) %>%
  # left_join(pk10r %>% select(CU, CU_NAME) %>% distinct(), by = c("CU_n" = "CU_n")) %>%
  # select(CU, CU_NAME, median, lower_95, upper_95) %>% 
  View()
